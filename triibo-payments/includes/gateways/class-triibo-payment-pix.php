...
class Triibo_Payment_Pix extends WC_Payment_Gateway
{
	/**
	...
	public function __construct()
	{
        ...
        $this->id                 = Triibo_Payments::DOMAIN . "-pix";
        $this->icon               = apply_filters(
			hook_name: "woocommerce_" . $this->id . "_icon",
			value    : plugins_url(
				path  : "assets/images/icon-pix.png",
				plugin: Triibo_Payments::FILE
			)
		);

        $this->method_title       = __( text: "Triibo PIX",                                  domain: $this->id );
        $this->method_description = __( text: "Pagamentos com PIX usando a Triibo Gateway.", domain: $this->id );
	...
	public function init_form_fields() : void
	{
		$this->form_fields = [
			...
			"enabled"     => [
				...
				"label"       => __( text: "Ativar Triibo PIX",                                         domain: $this->id ),
				"default"     => "no",
			],
			...
			"title"       => [
				...
				"default"     => __( text: "Triibo PIX",                                                domain: $this->id ),
			],
			"description" => [
				"type"        => "textarea",
				...
				"default"     => __( text: "Pague com a Triibo PIX",                                    domain: $this->id ),
			],
			"due_date"    => [
				"type"        => "number",
				"title"       => __( text: "Validade do PIX",                                           domain: $this->id ),
				"description" => __( text: "Informe qual o prazo de validade do PIX, em dias.<br>
									  Mínimo 1.",                                                       domain: $this->id ),
				"default"     => 2,
				"custom_attributes" => [
					"maxlength" => 2,
					"min"       => 1,
			...
			"message_pre"     => [
				...
				"default"     => __( text: "Ao finalizar a compra, você terá acesso ao QR Code e poderá pagar no seu aplicativo favorito do PIX.<br><br> * O pedido será confirmado somente após o pagamento ser confirmado.", domain: $this->id ),
			],
			"message_pos"     => [
				"type"        => "textarea",
				...
				"default"     => __( text: "Utilize o seu aplicativo favorito do PIX para ler o QR Code ou copie o código abaixo para efetuar o pagamento.", domain: $this->id ),
			],
	...
	public function process_payment( mixed $order_id ) : array
	{
		...
		$resp_pay    = $this->api->do_payment( method: "pix", token: $token, data: $paymentInfo );
	...
	public function email_instructions( object $order ) : void
	{
		...
		if ( $code && !empty( $code[ "invoice" ] ) )
		{
			wc_get_template(
				...
				args: [
					"type" => $code[ "type"    ],
					"link" => $code[ "invoice" ][ "qrCode"  ],
					"code" => $code[ "invoice" ][ "pixCode" ],
				],
				template_path: "",
	...
	/**
	 * Render pix code on thank_you/order page.
	 *
	 * @since 1.0.0
	 *
	 * @param array 	$invoice
	...
	private function render_code( array $invoice ) : void
	{
		...
		$link = $invoice[ "qrCode"  ];
		$code = $invoice[ "pixCode" ];

		?>
		<div class="tpb-pix-container" style="text-align:center; margin:20px 0;" >
			<div class="tpb-pix-instructions" >
				<?php echo $this->message_pos; ?>
			</div>
			<input type="hidden" value="<?php echo $code; ?>" id="copiar" >

			<img style="max-width:350px; width:50%; min-width:200px; max-height:350px; height:50%; min-height:200px;"
				src="data:image/gif;base64,<?php echo $link; ?>" />

			<p class="tpb-pix-p" style="font-size:14px; margin:.5em 0; word-break:break-all;" >Código PIX</p>
			<p class="tpb-pix-p" style="font-size:14px; margin:.5em 0; word-break:break-all;" ><?php echo $code; ?></p>

			<button class="button tpb-pix-button-copy-code" style="margin:1em auto;" onclick="copyCode()" >
				<?php echo __( text: "Copiar o código acima", domain: $this->id ); ?>
			</button>
			<div class="tpb-pix-response-output inactive" style="margin:1em .5em; padding:.2em 1em; border:2px solid #46b450; display:none;" aria-hidden="true" >
		...
		<script>
			function copyCode()
			{
				...
				if ( jQuery( "div.tpb-pix-response-output" ) )
					jQuery( "div.tpb-pix-response-output" ).show();
				else
